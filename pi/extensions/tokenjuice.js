 ...
 * generated by tokenjuice v0.8.5
 */
...
function hasMultipleSubstantiveShellCommands(input) {
...
var SSH_OPTIONS_WITH_VALUES = /* @__PURE__ */ new Set([
  "-B",
  "-b",
  "-c",
  "-D",
...
function isPlutilFileContentArgv(argv) {
...
function isReadOnlyConfigInspectionArgv(argv) {
  return getCommandName(argv) === "openclaw" && argv[1] === "config" && argv[2] === "get";
}
function getSshRemoteCommand(argv) {
...
function isVerbatimRemoteInspectionCommand(command) {
  const effectiveCommand = unwrapShellRunner({ command }) ?? command;
  const isSingleGhContentsDecode = isGhApiContentsDecodeCommand(effectiveCommand) && !hasSequentialShellCommands(effectiveCommand) && /^[^|]+\|\s*base64\s+(?:-[dD]\b|--decode\b)\s*$/u.test(effectiveCommand.trim());
  if (isSingleGhContentsDecode) {
    return true;
  }
  if (isCompoundShellCommand(stripLeadingCdPrefix(command)) || isCompoundShellCommand(effectiveCommand)) {
    return false;
  }
  const argv = getInspectionArgv({ command: effectiveCommand });
  return isPlutilFileContentArgv(argv) || isReadOnlyConfigInspectionArgv(argv) || isFileContentInspectionArgv(argv);
}
function isVerbatimConfigInspectionCommand(input) {
  ...
  return candidates.some((candidate) => isPlutilFileContentArgv(candidate.argv) || isReadOnlyConfigInspectionArgv(candidate.argv)) || candidates.some((candidate) => {
    const remoteCommand = getSshRemoteCommand(candidate.argv);
    return remoteCommand !== null && isVerbatimRemoteInspectionCommand(remoteCommand);
  });
}
...
var MAX_SHARD_COUNT = 100;
var MAX_STORE_FILES = 1e4;
var MAX_SEGMENT_BYTES = 64 * 1024 * 1024;
var DIRECTORY_ENTRY_SLACK = 256;
var DEFAULT_RETENTION_DAYS = 14;
var DEFAULT_SHARD_COUNT = 8;
var DEFAULT_MAX_SEGMENT_BYTES = 1024 * 1024;
function wait(ms) {
...
function shardFor(id, shardCount) {
  const digest = createHash("sha256").update(id).digest();
  return digest[0] % shardCount;
}
function segmentName(prefix, day, shard) {
  return `${prefix}-${day}-${String(shard).padStart(2, "0")}.jsonl`;
}
function parseSegmentDay(name, prefix) {
  const escapedPrefix = prefix.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
  return new RegExp(`^${escapedPrefix}-(\\d{4}-\\d{2}-\\d{2})-\\d{2}\\.jsonl$`, "u").exec(name)?.[1];
}
async function tryCreateLock(lockPath) {
  ...
  try {
    await writeFile(join(lockPath, "owner"), owner, { encoding: "utf8", flag: "wx", mode: 384 });
    return owner;
  } catch {
    return void 0;
  }
...
async function pruneExpiredSegments(directory, prefix, retentionDays, maxFiles, now) {
...
async function countOwnedSegments(directory, prefix, maxFiles) {
  const directoryHandle = await opendir(directory);
  let count = 0;
  let inspected = 0;
  try {
...
async function appendBoundedJsonl(directory, prefix, id, value, options = {}) {
  const { retentionDays, shardCount, maxSegmentBytes } = resolveStoreOptions(options);
  const now = options.now ?? /* @__PURE__ */ new Date();
  if (!Number.isFinite(now.getTime())) {
    throw new RangeError("now must be a valid Date");
  }
  const line2 = `${JSON.stringify(value)}
`;
  const lineBytes = Buffer.byteLength(line2, "utf8");
  ...
  try {
    ...
    await appendFile(path, line2, { encoding: "utf8", mode: 384 });
    return path;
  } finally {
    await releaseLock(lockPath, owner);
  }
...
var FLAG_EMOJI_PATTERN = new RegExp("^\\p{Regional_Indicator}{2}$", "u");
var KEYCAP_EMOJI_PATTERN = /^[#*0-9]\uFE0F?\u20E3$/u;
var REPEATED_EMOJI_BANNER_MIN_REPETITIONS = 24;
...
function isEmojiGrapheme(text) {
  return EMOJI_PATTERN.test(text) || FLAG_EMOJI_PATTERN.test(text) || KEYCAP_EMOJI_PATTERN.test(text);
}
function collapseRepeatedEmojiBanner(lines) {
  let collapsed = false;
  const nextLines = lines.map((line2) => {
    const visibleSegments = graphemes(line2).filter((segment) => !/^\s+$/u.test(segment));
    const repeatedEmoji = visibleSegments[0];
    if (visibleSegments.length < REPEATED_EMOJI_BANNER_MIN_REPETITIONS || !repeatedEmoji || !isEmojiGrapheme(repeatedEmoji) || visibleSegments.some((segment) => segment !== repeatedEmoji)) {
      return line2;
    }
    collapsed = true;
...
var STATS_ENABLED_ENV = "TOKENJUICE_STATS";
var METADATA_SEGMENT_DIRECTORY = "metadata-v1";
var METADATA_SEGMENT_PREFIX = "events";
var OPTIONAL_METADATA_STORAGE_ERROR_CODES = /* @__PURE__ */ new Set([
  "EACCES",
  "EDQUOT",
  "EEXIST",
  "EFBIG",
...
function getDefaultArtifactDir() {
  ...
  return join2(homedir(), ".tokenjuice", "artifacts");
}
...
function metadataSegmentDir(storeDir) {
  return join2(resolveArtifactBaseDir(storeDir), METADATA_SEGMENT_DIRECTORY);
}
var SAFE_COMMAND_FAMILY = /^[A-Za-z0-9][A-Za-z0-9._+-]{0,127}$/u;
function getTelemetryCommandFamily(input) {
  try {
    const family = getCommandName(getEffectiveCommandArgv(input));
    return family && SAFE_COMMAND_FAMILY.test(family) ? family : void 0;
  } catch {
    return void 0;
  }
}
function buildTelemetryMetadata(metadata, input) {
...
async function appendMetadataEvent(id, metadata, input, hasRaw, storeDir) {
...
function shouldRecordStats(env = process.env) {
  const value = env[STATS_ENABLED_ENV]?.trim().toLowerCase();
  return value !== "0" && value !== "false" && value !== "no" && value !== "off";
}
async function storeArtifact(input, storeDir) {
  const id = `tj_${randomUUID2().slice(0, 12)}`;
  const ref = buildArtifactPaths(id, storeDir);
  await mkdir2(resolveArtifactBaseDir(storeDir), { recursive: true, mode: 448 });
  ...
  await Promise.all([
    writeFile2(ref.path, input.rawText, { encoding: "utf8", mode: 384 }),
    writeFile2(ref.metadataPath, JSON.stringify(artifact.metadata, null, 2), { encoding: "utf8", mode: 384 })
  ]);
  if (input.recordStats ?? shouldRecordStats()) {
    await appendMetadataEvent(id, artifact.metadata, input.input, true, storeDir).catch(() => void 0);
  }
  return ref;
}
...
async function tryStoreArtifactMetadata(input, storeDir) {
  try {
...
function userRulesRoot(customDir) {
  return customDir ?? join3(homedir2(), ".config", "tokenjuice", "rules");
}
function projectRulesRoot(cwd, customDir) {
  return customDir ?? join3(cwd ?? process.cwd(), ".tokenjuice", "rules");
}
...
async function loadRuleDescriptorsFromRoot(root, source) {
  ...
  return await Promise.all(
    files.map(async (fullPath) => {
      const parsed = JSON.parse(await readFile3(fullPath, "utf8"));
...
var FAILURE_SEVERITY_FACTS = /* @__PURE__ */ new Set(["error", "warning"]);
...
function formatInline(classification, input, summary, facts, noOmit = false) {
  const neutralizeSeverityFacts = classification.matchedReducer === "generic/fallback" && input.exitCode === 0;
  const factParts = Object.entries(facts).filter(([, count]) => count > 0).map(([name, count]) => pluralize(
    count,
...
async function reduceExecutionWithRules(input, rules, opts = {}) {
  ...
  const resolvedMatch = opts.classifier || multipleSubstantiveCommands ? void 0 : resolveRuleMatch(input, rules);
  const classification = multipleSubstantiveCommands ? {
    family: "generic",
    confidence: 0.2,
    matchedReducer: "generic/fallback"
  } : resolvedMatch?.classification ?? classifyExecution(input, rules, opts.classifier);
  const reducerInput = multipleSubstantiveCommands ? normalizedInput : resolvedMatch?.candidateInput ?? normalizedInput;
  ...
  const requiresVerbatimOutput = !multipleSubstantiveCommands && isVerbatimConfigInspectionCommand(input);
    ...
    const rawRef2 = opts.store ? await storeArtifact(
      {
        ...
        rawText,
        classification,
        ...opts.recordStats !== void 0 ? { recordStats: opts.recordStats } : {},
        stats: {
          rawChars: measuredRawChars,
  ...
  const inspectionSummary = multipleSubstantiveCommands ? null : buildInspectionSummary(normalizedInput, rawText, opts.noOmit);
...
function getAutoCompactionEnabled(projectRoot) {
  ...
  const agentDir = process.env.PI_CODING_AGENT_DIR || join4(home, ".pi", "agent");
  const settingsPaths = [join4(agentDir, "settings.json"), join4(projectRoot, ".pi", "settings.json")];
  let enabled = true;
...
function createTokenjuicePiExtension(config) {
  ...
  return function tokenjuicePiExtension(pi) {
    ...
    async function loadFullOutputText(fullOutputPath) {
      let details;
      try {
        details = await stat2(fullOutputPath);
      } catch (error) {
      ...
      try {
        return await readFile4(fullOutputPath, "utf8");
      } catch (error) {
